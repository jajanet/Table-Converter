 ...
 * 
 * TODO:
 * comprehensive md support and read through https://github.com/adam-p/markdown-here/wiki/Markdown-Cheatsheet
 * csv, tsv and other file support
 * */
function upload(evt) {
    ...
    reader.onload = (() => {
      return (e) => {
        let tableArray = processText(e.target.result.replace(/(\r\n|\r\n\t|\n|\r)/gm,"\n"));
        return makeTable(tableArray)
      };
      })(f);
...
/**
 * translates formatted str into
 * 2d array that is used to create the table/sorting
*/
function processText(str, format="md"){
  // TODO: add more formats
  return processMarkdown(str)
}
...
function processMarkdown(str){
  ...
  // each elt is an str which represents a row
  // ignore \n char and nonalphanum str
  arr = arr.split(/(\n)/gm)
...
/**
 * resets the textarea in the demo to equal the defaukt demo str
 * */
function setDefaultTextArea(){ 
  document.getElementById('demo').value = getDemo()
  document.getElementById('demo').innerHTML = getDemo()
}

/**
 * returns true if str is a nonempty string with at least one alphanum char
*/
function hasAlphaNum(str){
  if(str.replace(/ /g, "").match(/[a-z, 0-9]/i)){ return true }
  return false
}

// when user sends an upload, this event listener triggers the processing
